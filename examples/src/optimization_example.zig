...
pub fn main() !void {
    ...
    std.debug.print("== Rosenbrock (global minimum at (1, 1)) ==\n", .{});
    {
        ...
        var res = try findGlobalOptimum(io, gpa, rosen, &variables, .{ .policy = .min, .max_evals = 150 });
    ...
    std.debug.print("== Be-like-target (BOBYQA's job; optimum at {{3, 5, 1, 7}}) ==\n", .{});
    {
        ...
        var res = try findGlobalOptimum(io, gpa, beLikeTarget, &variables, .{ .policy = .min, .max_evals = 200, .max_concurrency = 4 });
        defer res.deinit(gpa);
        printVec("  solution x = ", res.x);
        std.debug.print("  solution y = {d:.6}\n\n", .{res.y});
    }
